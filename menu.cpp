...
///SET
void Menu::Set_Start_Game(bool _Start_Game)
{
 Start_Game=_Start_Game;
}

void Menu::Set_Settings(bool _Settings)
{
 Settings=_Settings;
}

void Menu::Set_Exit_Game(bool _Exit_Game)
{
...
bool Menu::Get_Settings()
{
 return Settings;
}

bool Menu::Get_Exit_Game()
{
 return Exit_Game;
}

///MENU_ACTIONS
void Menu::Start_Menu(bool Start_Game,bool Settings,bool Exit_Game)///CE PARAMETRI PUN???
{
 if((keystates(SDL_SCANCODE_UP)==1 || keystates(SDL_SCANCODE_W)==1) && pos_menu>1)
    {
     if(pos_menu==2)//SETTINGS
        {
         Set_Settings(false);
         Set_Start_Game(true);
        }
 ...
 if((keystates(SDL_SCANCODE_DOWN)==1 || keystates(SDL_SCANCODE_S)==1) && pos_menu<MAX_OPTIONS)
    {
     if(pos_menu==1)//START GAME
        {
 ...
 /*if(Get_Start_Game()==true)
    //afisez imag de selected
 else
    {
     //afisez imag de neselected
    }*/
//}

#include "menu.h"

const int RESOLUTION_W=1366;
const int RESOLUTION_H=768;
...
void Menu_Option::Load(FILE *read_menu)
{
 int _r,_g,_b;
 fgets(text,sizeof text,read_menu);
 if(text[strlen(text)-1]=='\n')
    text[strlen(text)-1]=NULL;
 fscanf(read_menu,"%s %d",font_name,&font_size);
 fscanf(read_menu,"%d %d %d ",&_r,&_g,&_b);
 color.r=(Uint8)_r;
 color.g=(Uint8)_g;
 color.b=(Uint8)_b;
 fscanf(read_menu,"%d %d ",&screen_pos.x,&screen_pos.y);
...
void Menu_Option::Get_screen_pos()
{
 return screen_pos;
}
 ...
 if(!is_main_menu)
    {
     ...
     TTF_CloseFont(font);
     fscanf(read_menu,"%d %d ",&title_position.x,&title_position.y);
    }
 for(int i=0;i<number_of_options;i++)
     options[i].Load(read_menu);
 fclose(read_menu);
}

void MENU::Load1(const *filename)
{
 FILE *read_menu=fopen(filename,"r");
 fscanf(read_menu,"%d %d ",&number_of_options,&is_main_menu);
}
